...
class Ancora:
    ...
    def check_majority_rule_category(self, conjugated, infinitive, srl, pos_srl):
        majority_category = self.category(conjugated, infinitive)
        if majority_category != None:
            return self.check_category_argument_rule(majority_category, srl, pos_srl)
        return None 

    def check_unanimity_categories_argument_rule(self, conjugated, infinitive, srl, pos_srl):
        categories = self.categories(conjugated, infinitive)
        for category in categories:
            if not self.check_category_argument_rule(category, srl, pos_srl):
                return False
        return True

    def check_category_argument_rule(self, category, srl, pos_srl):
        verb_transitivity = VerbTransitivity()
        return verb_transitivity.check_category_argument_rule(category, srl, pos_srl)
